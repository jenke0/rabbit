"""
SmoothExtendedABCD background estimation param model.

Like ExtendedABCD, but the per-bin free parameters along one nominated smoothing
axis are replaced by an exponential Chebyshev polynomial:
...
This applies to all five free-parameter regions (A, B, C, Ax, Bx), reducing
parameters from 5·n_bins to 5·n_outer·(order+1).
...
class SmoothExtendedABCD(ParamModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Build outer × smooth index structure for all six regions
        all_regions = ["A", "B", "C", "D", "Ax", "Bx"]
        all_channels = [
            channel_A,
            channel_B,
            channel_C,
        ...
        for region, ch_dict in zip(all_regions, all_channels):
        ...
        mc_Ax = _gather_flat(outer_global_indices["Ax"])
        mc_Bx = _gather_flat(outer_global_indices["Bx"])
        ...
        self._idx = {
            r: _build_scatter_idx(outer_global_indices[r]) for r in all_regions
        }
        ...
        # Parameter names: region × outer_bin × poly_coeff (for the 5 free regions)
        free_regions = ["A", "B", "C", "Ax", "Bx"]
        free_channels = [channel_A, channel_B, channel_C, channel_Ax, channel_Bx]
        names = []
        for region, ch_dict in zip(free_regions, free_channels):
    ...
    def compute(self, param, full=False):
        ...
        Args:
            param: 1D tensor of length 5 * n_outer * (order+1).
        ...
        params_A = tf.reshape(
            param[0 * n_coeffs : 1 * n_coeffs], [self.n_outer, self.order + 1]
        )
        params_B = tf.reshape(
            param[1 * n_coeffs : 2 * n_coeffs], [self.n_outer, self.order + 1]
        )
        params_C = tf.reshape(
            param[2 * n_coeffs : 3 * n_coeffs], [self.n_outer, self.order + 1]
        )
        params_Ax = tf.reshape(
            param[3 * n_coeffs : 4 * n_coeffs], [self.n_outer, self.order + 1]
        )
        ...
        ax = tf.exp(tf.matmul(params_Ax, self.basis))
        bx = tf.exp(tf.matmul(params_Bx, self.basis))

        # Flatten to [n_outer * n_smooth] = [n_total_bins per region]
        a_flat = tf.reshape(a, [-1])
        b_flat = tf.reshape(b, [-1])
        c_flat = tf.reshape(c, [-1])
        ax_flat = tf.reshape(ax, [-1])
        bx_flat = tf.reshape(bx, [-1])
